...
```{r}
str(gapminder) #str=structure of the dataset 
...
# Make scientific notations go away.
gapminder %>% 
  filter(year==1952) %>% 
...
We use it to transform the x-axis to display values on a logarithmic scale rather than a linear one. 

2. *Answer: In Figure 1: Who is the outlier (the richest country in 1952) far right on the x axis?* 
The easy way is to find the row with the highest value in gdpPercap.
gapminder %>%
  filter(year == 1952) %>%
  arrange(desc(gdpPercap)) %>%
  head(1)
...
We are just using the same commando as we did in assigment 2. 
gapminder %>%
  filter(year == 2007) %>%
  arrange(desc(gdpPercap)) %>%
  head(5)
...
anim <- ggplot(gapminder, aes(gdpPercap, lifeExp, size = pop/1000000)) +
   geom_point(aes(color=continent)) +
  scale_x_log10() +
  ggtitle("World development from 1952-2009")+
  labs(x="GBP per capita", 
       y= "Life expectancy")+
  theme(text=element_text(size = 18))+
  theme_dark()
...
gapminder %>%
  filter(year %in% c(2002, 2007), country=="Lesotho") %>%
  group_by(year) %>%
  summarise(
